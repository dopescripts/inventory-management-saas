...
export default function Create({ items, warehouses, directions }: { items: Item[], warehouses: Warehouse[], directions: Direction[] }) {
    ...
    const selectedWarehouse = useMemo(() => warehouses.find(w => w.id.toString() === data.warehouse_id), [warehouses, data.warehouse_id]);
            ...
            <div className='flex h-full flex-1 flex-col gap-4 rounded-xl p-4 md:p-6'>
                <div className='max-w-2xl'>
                    <div className='mb-6'>
                        ...
                        <p className="text-muted-foreground mt-1">Record a manual increase or decrease in stock level.</p>
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field className="md:col-span-2">
                                ...
                                <FieldError errors={[{ message: errors.item_id }]} />
                            </Field>

                            <Field>
                                <FieldLabel htmlFor="warehouse_id">Warehouse</FieldLabel>
                                <Select value={data.warehouse_id} onValueChange={(v) => setData(data => ({...data, warehouse_id: v, location_id: ''}))}>
                            ...
                            <Field>
                                <FieldLabel htmlFor="location_id">Location (Optional)</FieldLabel>
                                <Select value={data.location_id} onValueChange={(v) => setData('location_id', v)} disabled={!data.warehouse_id || locations.length === 0}>
                                    <SelectTrigger id="location_id">
                                        <SelectValue placeholder={locations.length === 0 && data.warehouse_id ? "No locations found" : "Select location"} />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <FieldError errors={[{ message: errors.location_id }]} />
                            </Field>

                            <Field>
                                <FieldLabel htmlFor="direction">Adjustment Type</FieldLabel>
                                ...
                                <FieldError errors={[{ message: errors.direction }]} />
                            </Field>
                            ...
                            <Field className="md:col-span-2">
                                ...
                                <Select value={data.reason} onValueChange={(v) => setData('reason', v)}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="Count Discrepancy">Count Discrepancy</SelectItem>
